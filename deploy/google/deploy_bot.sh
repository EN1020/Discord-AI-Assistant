...
if ! command -v git >/dev/null 2>&1; then
  echo "git is required. Run deploy/google/bootstrap_ubuntu.sh first." >&2
  exit 1
fi

if ! command -v docker >/dev/null 2>&1; then
  echo "docker is required. Run deploy/google/bootstrap_ubuntu.sh first." >&2
  exit 1
fi
...
Edit .env on this Google Cloud VM and fill in:
  DISCORD_TOKEN
  OPENAI_API_KEY
